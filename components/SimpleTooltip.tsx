    ...
    if (isVisible && tooltipRef.current && containerRef.current) {
      ...
      if (isMobile) {
        ...
        if (centerX - tooltipHalfWidth < 10) {
          ...
          newPosition =
            rect.top - tooltipRect.height - 10 < 0 ? 'bottom' : 'top';
      ...
      <AnimatePresence>
        {isVisible && (
          <motion.div
            ...
            className={`absolute ${getPositionClasses()} pointer-events-none z-[9999]`}
            exit={{
              opacity: 0,
              scale: 0.8,
          ...
          >
            <div className="w-[260px] rounded-lg border border-white/20 bg-gray-900/95 px-6 py-4 text-white text-xs leading-relaxed shadow-xl backdrop-blur-sm sm:w-[300px] sm:text-sm md:w-[320px] md:text-sm lg:w-[340px] lg:text-base">
              {content}
            </div>
            {/* Arrow */}
