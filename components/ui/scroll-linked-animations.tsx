...
export default function ScrollLinkedAnimations({ children, className = '' }: ScrollLinkedAnimationsProps) {
...
export function ScrollLinkedBackground() {
  ...
  return (
    <motion.div
      ...
      style={{
        backgroundImage: 'radial-gradient(circle at center, rgba(30, 155, 113, 0.3) 0%, transparent 70%)',
...
export function ScrollLinkedTextReveal({ children, className = '' }: ScrollLinkedAnimationsProps) {
  const containerRef = useRef<HTMLDivElement>(null)
  const { scrollYProgress } = useScroll({
    target: containerRef,
...
export function ScrollLinkedCounter({ target = 100, className = '' }: { target?: number, className?: string }) {
  const containerRef = useRef<HTMLDivElement>(null)
  const { scrollYProgress } = useScroll({
    target: containerRef,
    offset: ["start end", "end start"]
