        ...
        whileHover={(() => {
          const isVertical =
            ...
            (position.includes('center') &&
              (position.includes('left') || position.includes('right')));
          const isRightSide =
            position === 'bottom-right' || position.includes('right');
        ...
        <AnimatePresence mode="wait">
          {isHovered && (
            <motion.div
              ...
              className={`absolute z-50 whitespace-nowrap rounded-lg border border-white/20 bg-gray-900/95 px-3 py-1.5 text-white text-xs shadow-xl backdrop-blur-lg ${
                position === 'bottom-right'
                  ? '-left-24 -translate-y-1/2 top-1/2 transform'
                  : position.includes('center') &&
                      (position.includes('left') || position.includes('right'))
                    ? position.includes('right')
                      ? '-left-24 -translate-y-1/2 top-1/2 transform'
                      : '-right-24 -translate-y-1/2 top-1/2 transform'
                    : '-top-12 -translate-x-1/2 left-1/2 transform'
              }`}
            ...
            >
              ...
              <div
                className={`absolute h-0 w-0 ${
                  position === 'bottom-right'
                    ? '-translate-y-1/2 top-1/2 left-full transform border-transparent border-t-4 border-b-4 border-l-4 border-l-gray-900/95'
                    : position.includes('center') &&
                        (position.includes('left') ||
                          position.includes('right'))
                      ? position.includes('right')
                        ? '-translate-y-1/2 top-1/2 left-full transform border-transparent border-t-4 border-b-4 border-l-4 border-l-gray-900/95'
                        : '-translate-y-1/2 top-1/2 right-full transform border-transparent border-t-4 border-r-4 border-r-gray-900/95 border-b-4'
                      : '-translate-x-1/2 top-full left-1/2 transform border-transparent border-t-4 border-t-gray-900/95 border-r-4 border-l-4'
                }`}
              />
            </motion.div>
          )}
          ...
          {isExpanded && (
            ...
            >
              {items.map((item, index) => (
                <DockItem index={index} item={item} key={item.id} />
              ))}
            </motion.div>
          )}
        </AnimatePresence>
