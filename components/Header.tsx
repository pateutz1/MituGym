...
const Header = () => {
  ...
  return (
    <>
      {/* Desktop Navigation */}
      <motion.header
        className="fixed top-3 left-0 right-0 z-50 hidden md:block"
      ...
      <>
        <motion.header
          className="fixed top-4 left-0 right-0 z-50 md:hidden px-4 mobile-header"
          initial={{ y: -100, opacity: 0 }}
          animate={{ y: 0, opacity: 1 }}
          transition={{ duration: 0.8, ease: "easeOut" }}
        >
          ...
          >
            ...
            <div className="flex items-center justify-between px-5 py-3 relative z-10">
              ...
              {/* Mobile menu button */}
              <motion.button
                className="text-white p-2 hover:bg-white/10 rounded-lg transition-all duration-300 hover:text-primary touch-manipulation"
                onClick={(e) => {
                  e.stopPropagation();
                  ...
                  <div className="flex flex-col space-y-1 px-5 py-3">
                    {navItems.map((item, index) => (
                      ...
                      >
                        <Link
                          href={item.href}
                          className="text-gray-200 hover:text-primary hover:bg-primary/10 transition-all duration-300 font-medium font-display py-2.5 px-3 rounded-lg text-sm tracking-wide block relative group touch-manipulation"
                          onClick={(e) => {
